...
######################################################
#Nombre: Mireles Quintero José Manuel
#Cuenta: 311197387

#----------------------------------------------------#
#1. (16 de marzo) 
#Analizar la deducción de la cópula subyacente en el Ejemplo 3.3 
#del libro de Nelsen y calcular en función del parámetro de dicha cópula 
#las medidas de dependencia de Schweizer-Wolff, Hoeffding y distancia supremo, 
#así como las medidas de concordancia de Kendall, Spearman y Erdely. Luego graficar 
#juntas todas las medidas de dependencia y concordancia como funciones del parámetro.

#Cargar librería "cubature" para integrar
list.of.packages <- c("cubature")
...
curve(Medida1a,0,1,xname="SW",col=5,xlab="Medidas de dependencia")
...
curve(Medida2a,0,1,xname="H",col=6,xlab="Medida de dependencia",add=T)
...
curve(Medida3a,0,1,n=300,col=8,xname="Sup",xlab="Medida de dependencia",add=T)
...
curve(Concordancia1a,0,1,col=5,xname="K",xlab="Medida de concordancia")
...
curve(Concordancia2a,0,1,xname="Sp",col=6,xlab="Medida de concordancia",add=T)
...
curve(Concordancia3a,0,1,xname="Er",col=8,xlab="Medida de concordancia",add=T)
...
#----------------------------------------------------#
#2. (17 de marzo)
#Considere un vector aleatorio (X,Y) con función de densidad conjunta de 
#probabilidades del Ejemplo 1.7 de las notas sobre vectores aleatorios. Programando en R:
...
#b) Con los valores simulados de X obtenga un histograma en la escala 
#adecuada para que encima grafique la densidad teórica marginal de X. Lo mismo para Y.
...
#c) Obtenga gráficas de los conjuntos de nivel de la cópula 
#subyacente C mediante las funciones contour e image.
...
#e) Calcule las medidas de dependencia de Schweizer-Wolff, Hoeffding y 
#distancia supremo, así como las medidas de concordancia de Kendall, Spearman y Erdely.
...
#----------------------------------------------------#
#3. (25 de marzo)
#Simule una muestra aleatoria de tamaño n = 3000 a partir de un vector 
#aleatorio (U,V) con marginales Uniformes(0,1) y cópula Clayton con parámetro 2.
...
#Calcule las medidas de dependencia de Schweizer-Wolff, Hoeffding 
#y distancia supremo, así como las medidas de concordancia de Kendall, Spearman y Erdely.
linea <- seq(0,1,.01)
cop <- matrix(nrow = length(linea), ncol = length(linea))
...
#----------------------------------------------------#
#4. (25 de marzo)
#Lo mismo del ejercicio anterior pero para (X,Y) pero con distribuciones 
#marginales X ~ Pareto(1,1), Y ~ Cauchy(0,1).
...
plot(X,Y, xlim = c(-1,60),ylim = c(-15,15),main = "Gráfica de dispersión (X,Y)")


#Calcule las medidas de dependencia de Schweizer-Wolff, Hoeffding y distancia
#supremo, así como las medidas de concordancia de Kendall, Spearman y Erdely.
linea <- seq(0,1,.01)
cop <- matrix(nrow = length(linea), ncol = length(linea))
W <- matrix(nrow = length(linea), ncol = length(linea))
